...
A sophisticated RL-based swing trading bot tailored for both long and short positions, integrating:
- Alpaca API for stock and ETF trading
- SuperTrend signals for bullish/bearish direction
- Advanced DQN RL agent with Double DQN, Dueling Networks, and Prioritized Experience Replay
- Extensive Feature Engineering with RSI, MACD, Bollinger Bands, SMA Crossover, SuperTrend, Volatility, Volume Indicators
- Enhanced Reward Function considering Directional Accuracy and Penalties for Losses
- Robust Risk Management with Dynamic Position Sizing and Trailing Stop-Losses/Take-Profit
...
- Novel Unified Multi-Symbol RL Agent with Specialized Clusters
"""
...
class SymbolListScanner:
    ...
    async def scan(self) -> List[str]:
        ...
        try:
            ...
            if len(symbols) != 10:
                self.logger.warning(f"Symbol list should contain exactly 10 symbols (5 stocks and 5 ETFs). Current count: {len(symbols)}")
...
class DQNAgent:
    def __init__(self, in_dim: int, out_dim: int, num_clusters: int, device: torch.device, memory_size: int, batch_size: int,
    ...
    def store_transition(self, s: torch.Tensor, a: int, r: float, ns: torch.Tensor, d: int, cluster_id: int):
        """
    ...
    def load_checkpoint(self, filepath: str):
        ...
        if not os.path.exists(filepath):
            ...
            return
        try:
            # To address the FutureWarning, set weights_only=True if only weights are being loaded.
            # However, since we're loading optimizer state and other components, we keep weights_only=False.
...
def compute_technical_indicators(df: pd.DataFrame) -> pd.DataFrame:
    """
    Compute SuperTrend, RSI, MACD, Bollinger Bands, SMA Crossover, Volatility, and Volume Indicators.
    """
    if df.empty:
        return df
...
class TradeState:
    ...
    def update_trade(self, symbol: str, entry_price: float):
        """
        Initialize or update trade state with trailing stop-loss and take-profit levels.
        """
        now = datetime.now(timezone.utc)
    ...
    def adjust_stop_loss(self, symbol: str, current_price: float):
        ...
        if symbol in self.active_trades:
            ...
            # Only adjust for long positions
            if current_price > trade['entry_price']:
                new_stop_loss = current_price * (1 - TRAILING_STOP_LOSS)
...
def get_reward(symbol: str, eq_now: float) -> float:
    """
    Calculate reward based on change in equity.
    """
...
class RLTrader:
    def __init__(self, device: torch.device, logger: logging.Logger, checkpoint_path: str = "agent_checkpoint.pth",
        ...
        self.device = device
        self.state_dim = 14  # Expanded state dimensions with additional indicators
        self.action_dim = 3  # 0=Hold, 1=Buy (Long), 2=Sell (Close Long)
        self.checkpoint_path = checkpoint_path
        self.replay_memory_path = replay_memory_path
        self.num_clusters = 10  # One cluster per symbol
        self.agent = DQNAgent(
    ...
    def pick_action(self, state: torch.Tensor, cluster_id: int) -> int:
        return self.agent.select_action(state, cluster_id)

    def store_transition(self, s: torch.Tensor, a: int, r: float, ns: torch.Tensor, d: int, cluster_id: int):
        self.agent.store_transition(s, a, r, ns, d, cluster_id)
...
async def apply_action(symbol: str, action: int, trader: AlpacaTrader, logger: logging.Logger, trade_state: TradeState):
    """
    ...
    if shares < 1:
        logger.info(f"Not enough equity to allocate to {symbol}. Required shares: 1, Available allocation: {max_alloc}")
        return
    ...
    if action == 0:
        ...
        if pos and float(pos.qty) > 0:
            ...
            if order:
                logger.info(f"Sell order executed for {symbol}, Shares: {shares}")
                trade_details = {
                    "timestamp": datetime.now(timezone.utc).isoformat(),
                    "symbol": symbol,
                    "action": "Sell",
...
def build_state(df: pd.DataFrame, symbol: str, trade_state: TradeState) -> torch.Tensor:
    ...
    # Equity (using active trade entry price; 0 if no active trade)
...
###############################################################################
async def process_symbol(symbol: str, rl: RLTrader, trader: AlpacaTrader, logger: logging.Logger, trade_state: TradeState, symbol_id: int):
    """
    ...
    # 4) Pick action
    action = rl.pick_action(state, symbol_id)
    logger.info(f"{symbol} => Action: {action}")

    # 5) Apply action
    await apply_action(symbol, action, trader, logger, trade_state)
    ...
    exit_reason = trade_state.check_exit(symbol, current_price, current_time, MIN_HOLD_DAYS, MAX_HOLD_DAYS) if current_price else None
    if exit_reason:
    ...
    # 12) Store transition
    rl.store_transition(state, action, reward, next_state, done, symbol_id)
...
async def run_bot():
    ...
    # Check if the number of symbols matches the number of clusters
    if len(symbols) != 10:
        logger.warning(f"Number of symbols ({len(symbols)}) does not match number of clusters (10). Adjusting clusters accordingly.")
